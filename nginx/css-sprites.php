...
$output = fopen(dirname(__FILE__) . '/web/public/css/sprites.css', 'w');
$total_height = 0;
$full_filenames = array();
foreach($sprites as $sprite) {
	$directory = opendir(dirname(__FILE__) . "/web/public/img/$sprite/");
	$filenames = array();
	...
	foreach($filenames as $filename) {
		...
		$size = getimagesize(dirname(__FILE__) . "/web/public/img/$sprite/$filename");
		$width = $size[0];
		$height = $size[1];
...
chdir(dirname(__FILE__) . '/web/public/img/');
