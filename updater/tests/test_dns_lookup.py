...
def test_non_ipv4_returns_unchanged() -> None:
...
def test_memcached_hit() -> None:
    mock_client = MagicMock()
    mock_client.get.return_value = "cached.example.com"
    assert lookup("9.9.9.9", memcached_client=mock_client) == "cached.example.com"
    mock_client.get.assert_called_once_with("torstatus_host_9.9.9.9")


def test_memcached_miss_then_lookup() -> None:
    _lookup_once.cache_clear()
    mock_client = MagicMock()
    mock_client.get.return_value = None
    with patch("socket.gethostbyaddr", return_value=("host.example.com", [], [])):
        assert lookup("9.9.9.9", memcached_client=mock_client) == "host.example.com"
